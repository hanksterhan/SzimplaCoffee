...
def _record_promo(session: Session, merchant: Merchant, homepage_url: str, html: str, confidence: float) -> None:
    text = _clean_text(html)
    threshold = _extract_free_shipping_threshold(text)
    if threshold:
        promo = PromoSnapshot(
...
def _crawl_shopify(session: Session, merchant: Merchant) -> CrawlSummary:
    ...
    with httpx.Client(follow_redirects=True, timeout=20.0, headers=headers) as client:
        ...
        _record_promo(session, merchant, merchant.homepage_url, homepage.text, 0.9)
        ...
        for raw in products:
            ...
            product = _upsert_product(
                ...
                str(raw["id"]),
                {
                    ...
                    "tasting_notes_text": _extract_field(text, "Tastes Like"),
                    "is_single_origin": "single origin" in " ".join(tags).lower() or "single origin" in text.lower(),
...
def _crawl_woocommerce(session: Session, merchant: Merchant) -> CrawlSummary:
    ...
    with httpx.Client(follow_redirects=True, timeout=20.0, headers=headers) as client:
        ...
        _record_promo(session, merchant, merchant.homepage_url, homepage.text, 0.7)
            ...
            for raw in raw_products:
                ...
                product = _upsert_product(
                    ...
                    str(raw["id"]),
                    {
                        ...
                        "tasting_notes_text": _extract_field(description, "Tastes Like"),
                        "is_single_origin": "single origin" in " ".join(categories).lower() or "single origin" in description.lower(),
...
def _crawl_generic(session: Session, merchant: Merchant) -> CrawlSummary:
    ...
    with httpx.Client(follow_redirects=True, timeout=20.0, headers=headers) as client:
        ...
        _record_promo(session, merchant, merchant.homepage_url, homepage.text, 0.4)
