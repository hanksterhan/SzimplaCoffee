...
// Carries callbacks needed to drive value+open changes from SelectItem.onClick
// when Radix's pointerup-based selection is swallowed by React 19.
type SelectShimCtx = {
...
>(({ className, children, position = "popper", ...props }, ref) => {
  ...
  // Outside-click shim: Radix uses pointerdown on document for dismiss but
  // React 19 may not deliver it reliably through the portal. We attach a
  // mousedown listener on document after the next animation frame (so the
  // opening click doesn't immediately retrigger it) and close if the target
...
>(({ className, children, onClick, ...props }, ref) => {
  ...
  return (
    <SelectPrimitive.Item
      ...
      onClick={(e) => {
        // Drive value+close directly via onClick — Radix uses pointerup for
        // mouse selection which React 19 swallows on portal content.
